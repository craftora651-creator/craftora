...
const VipSelection: React.FC = () => {
  ...
  const plans: Plan[] = [
    ...
    },
    {
      ...
      badge: 'MOST POPULAR',
      description: 'Advanced features designed to maximize your team\'s output and collaborative potential.',
      buttonText: 'Start Free Trial',
      buttonLink: '/subscribe/advanced',
      isPopular: true,
      features: [
        { text: 'Unlimited Team Workspaces', included: true },
        { text: '50GB High-speed Cloud Storage', included: true },
        { text: 'Advanced Real-time Analytics', included: true },
        { text: 'Priority 24/7 Priority Support', included: true },
        { text: 'Custom Domain & SSL Integration', included: true },
        { text: 'API Access & Webhooks', included: true }
      ],
      gradient: 'linear-gradient(145deg, #0D0F1F, #1A1D35)',
      shadowColor: 'rgba(0, 163, 255, 0.25)'
    },
    {
      id: 'enterprise',
      title: 'Enterprise',
  ...
  if (planId === 'advanced') {
    navigate('/shop');  // 🎯 Advanced'e tıklayınca SHOP'A GİT!
    ...
    <div className="vip-container">
      ...
      <div className="vip-header">
        <h1 className="vip-title">
          Ready to scale your <span className="vip-title-gradient">productivity?</span>
        </h1>
        <p className="vip-subtitle">
          Choose the perfect plan for your team. From solo creators to global enterprises, 
          we have a solution that fits your workflow and budget.
        </p>
      </div>
      ...
      <div className="vip-cards">
        {plans.map((plan) => (
          <div
            key={plan.id}
            className={`vip-card ${plan.isPopular ? 'vip-card-popular' : ''} ${hoveredCard === plan.id ? 'vip-card-hovered' : ''}`}
          ...
          >
            ...
            {/* Badge */}
            {plan.badge && (
              <div className={`vip-card-badge ${plan.isPopular ? 'vip-card-badge-popular' : ''}`}>
                {plan.badge}
              </div>
            )}
            ...
            <div className="vip-card-content">
              ...
              <button className="vip-card-learn-more">
                LEARN MORE →
              </button>
            </div>
          </div>
